...
def create_dynamic_nodes(base_class):
	...
	for(node_name,info)in rice_prompt_info.choice_classname_map.items():class_name=node_name;category='RiceRound/Advanced/Choice';dynamic_class=type(class_name,(base_class,),{'__node_name__':node_name,'CATEGORY':category});dynamic_classes[class_name]={_C:dynamic_class,_B:info.get(_B,class_name)}
...
NODE_DISPLAY_NAME_MAPPINGS={_D:'Simple Choice',_E:'Advanced Choice',_F:'Debug',_G:'Image Bridge',_H:'Simple Image',_I:'Download Image',_J:'Random Seed',_K:'Input Text',_L:'Mask Bridge',_M:'Download Mask',_N:'RiceRound Int',_O:'RiceRound Float',_P:'RiceRound Boolean',_Q:'RiceRound Str To Int',_R:'RiceRound Str To Float',_S:'RiceRound Str To Boolean',_T:'Decrypt',_U:'Output Image Bridge',_V:'Image URL',_W:'Upload Image',_X:'Output Mask Bridge',_Y:'Output Int',_Z:'Output Float',_a:'Output Boolean',_b:'Output Text',_c:'Encrypt',_d:'Output Image',_e:'Temp Token',**{name:cls[_B]for(name,cls)in dynamic_choice_nodes.items()}}
WEB_DIRECTORY='./js'
...
async def open_selector_list_folder(request):
	if request.remote not in(_f,'::1'):return web.json_response({_A:_g},status=403)
	choice_server_folder=RicePromptInfo().get_choice_server_folder()
	if not choice_server_folder.exists():return web.json_response({_A:'Folder does not exist'},status=404)
	...
	except Exception as e:return web.json_response({_A:str(e)},status=500)
